...
output_path = project_path / "results" / "MGE_files_contig" / directory
output_path_simple = project_path / "results" / "MGE_files_contig_simple" / directory
output_path_intermediate = project_path / "results" / "MGE_files_contig_intermediate" / directory
...
def detect_mges_simple(frame, n_threshold):
    i=0
    start_genes=[]
    end_genes=[]
    for index,row in frame.iterrows():
...
def generate_output_simple(genome_contig_file, n_threshold):
    """Generates csv files, containing a columns with the contig, a column with a number indicating an individual MGE, and a columns with gene_nrs, indicating the genes that belong to a particular MGE."""
    contig_extension=genome_contig_file.split('-')[1]
    contig=str(contig_extension.split('.')[0]+'.'+contig_extension.split('.')[1])
    MGE_list=[]
    mob_frame=pd.read_csv(input_path / genome_contig_file, index_col=0)
    start_genes, end_genes=detect_mges_simple(mob_frame, n_threshold)
    for i in range(0,len(start_genes)):
        for gene in range(start_genes[i], end_genes[i]+1):
            MGE_list.append({'contig':contig,'MGE':i+1,'gene_nr':gene})
    MGE_frame = pd.DataFrame.from_records(MGE_list)
...
if (method=="sophisticated"):
    ...
    score_threshold=sys.argv[3].split()[0]
    n_threshold=sys.argv[3].split()[1]
    fraction_threshold=sys.argv[3].split()[2]
    n_scc_threshold=sys.argv[3].split()[3]
    ...
    score_threshold=sys.argv[3].split()[0]
    n_threshold=sys.argv[3].split()[1]
    initial_n_threshold=sys.argv[3].split()[2]
    distance_threshold=sys.argv[3].split()[3]
    ...
    n_threshold=sys.argv[3]
    generate_output_simple(genome_contig_file, n_threshold=n_threshold)
