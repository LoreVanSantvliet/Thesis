...
def detect_mges_initial(frame, score_threshold, initial_n_threshold):
    # detection of initial (small) mobile regions
    i=0
    start_genes=[]
    end_genes=[]
    for index,row in frame.iterrows():
        ...
        if consecutive:
            i=i+1
            # next lines: if end of genome is reached, check if last part is an initial mobile region
            if index == len(frame)-1:
            ...
            # save initial mobile regions that are long enough, and/or reset variables
            if i>=int(initial_n_threshold):
                start_genes.append(index+1-i)
                end_genes.append(index)
                i=0
...
# problem: distance threshold
def join_mges_initial(start_genes, end_genes, n_threshold, distance_threshold):
    indices_to_join=[]
    for index in range(len(start_genes)):
        if index != 0:
            # save the index of the elements that need to be joined (the index of the second of these two elements)
            if(start_genes[index]<=end_genes[index-1]+int(distance_threshold)):
                indices_to_join.append(index)
    
    # for each index (starting at the last one), remove this from the start genes, and remove the previous one from the ned genes (=joining 2 elements)
    for index in indices_to_join[::-1]:
        start_genes.remove(start_genes[index])
        #end_genes[index]=end_genes[index-1]
        end_genes.remove(end_genes[index-1])
    
    # if the resulting element is not large enough: remove (from start genes and from end genes)
