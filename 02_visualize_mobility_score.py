...
def mobility_plot(genomes, genome, full, order, method="mobility_score", n=running_average_default, indicate_accessory=True):
...
def mobility_file(order, genome, genomes, full):
    for contig in get_contigs(order, genome):
        ...
        mob_order = pd.merge(genome_order, mobility_frame.loc[:,["orthogroup", "accessory_fraction"]], on="orthogroup", how="left").sort_values(by="order")
